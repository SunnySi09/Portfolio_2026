...
export default function StoreAdminCaseStudyPage() {
  ...
  return (
    <>
      ...
      <div className="absolute top-[72px] left-0 right-0 h-[380px] bg-gradient-to-b from-rausch/5 to-transparent pointer-events-none z-0" />
      ...
      <main className="relative z-10 shell pt-8 pb-24">
        ...
        <section className="mb-12 border-b border-hairline pb-8">
          <Reveal>
            <span className="eyebrow">Merchant Experience · Dashboard UX</span>
            <h1 className="mt-3 font-display text-4xl sm:text-6xl font-extrabold tracking-tight text-ink leading-tight">
              Case Study: Store Admin Dashboard
            </h1>
            <p className="mt-6 text-lg sm:text-xl leading-relaxed text-body max-w-3xl">
              A multi-tenant SaaS ERP portal enabling retail jewelry partners to customize storefront landing pages, manage catalog imports, and process order resizing queues under a centralized component design.
            </p>
          </Reveal>
          ...
          <Reveal delay={100}>
            <div className="mt-10 grid gap-6 sm:grid-cols-4 border-t border-hairline pt-8 text-xs">
              <div>
                ...
                <span className="text-[14px] font-semibold text-ink">Product &amp; UI/UX Designer</span>
                <p className="text-[10px] text-muted-soft mt-1 leading-normal">
                  Information Architecture, Wireframing, High-Fidelity UI, Product Guidelines, Design Handoff.
                </p>
              </div>
              <div>
                ...
                <p className="text-[10px] text-muted-soft mt-1 leading-normal">Active development sprint.</p>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {["Figma", "FigJam", "Claude", "Antigravity", "Framer"].map((t) => (
                    <span
                      key={t}
                      className="inline-flex rounded-full border border-rausch/20 bg-rausch/5 px-2 py-0.5 font-semibold text-[10px] text-rausch-active"
                    >
                      {t}
                    </span>
                  ))}
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left text-xs px-3 py-2 rounded transition-all duration-150 block truncate ${
                      isActive
                        ? "bg-rausch/10 text-rausch-active font-bold border-l-2 border-rausch"
            ...
            {caseStudySections.map((sec, idx) => (
              ...
              >
                <Reveal delay={idx === 0 ? 50 : 0}>
                  ...
                  {/* Inline interactive sandbox mockup injected inside 'Screen Walkthroughs' section */}
                  {sec.id === "screen-walkthroughs" && (
                    <div className="mt-8 border border-hairline rounded-xl bg-canvas p-2 sm:p-4 shadow-lift">
                      <div className="mb-3 flex items-center justify-between border-b border-hairline pb-2 px-1">
                        ...
                        <span className="text-[10px] font-mono text-muted uppercase font-semibold">
                          Interactive Store Admin Access Permissions Workspace
                        </span>
                      </div>
                      <CaseStudyPreview slug="store-admin-dashboard" />
                    </div>
                  )}
                </Reveal>
              </section>
