...
export default function IndustrialWebsitesCaseStudyPage() {
  ...
  return (
    <>
      ...
      <div className="sticky top-[72px] left-0 right-0 h-1 bg-[#ebe5d3] z-40">
        <div
          className="h-full bg-plum transition-all duration-75"
          style={{ width: `${scrollProgress}%` }}
        />
      </div>

      {/* Decorative Brand Top Wash */}
      <div className="absolute top-[72px] left-0 right-0 h-[380px] bg-gradient-to-b from-plum/5 to-transparent pointer-events-none z-0" />
      ...
      <main className="relative z-10 shell pt-8 pb-24">
        ...
        <section className="mb-12 border-b border-hairline pb-8">
          <Reveal>
            <span className="eyebrow">Corporate Web Design · Lead Generation</span>
            <h1 className="mt-3 font-display text-4xl sm:text-6xl font-extrabold tracking-tight text-ink leading-tight">
              Case Study: JK Velvet B2B Platform
            </h1>
            <p className="mt-6 text-lg sm:text-xl leading-relaxed text-body max-w-3xl">
              Redesigning the B2B web catalog, technical specs interface, and Request-For-Quote (RFQ) pipeline for a premium velvet fabric manufacturer.
            </p>
          </Reveal>
          ...
          <Reveal delay={100}>
            <div className="mt-10 grid gap-6 grid-cols-2 sm:grid-cols-5 border-t border-hairline pt-8 text-xs">
              <div>
                ...
                <span className="text-[14px] font-semibold text-ink">Lead UI/UX Web Designer</span>
                <p className="text-[10px] text-muted-soft mt-1 leading-normal">
                  Information Architecture, Wireframing, High-Fidelity UI, Asset Design, Visual QA.
                </p>
              </div>
              <div>
                <span className="block font-bold uppercase tracking-wider text-muted mb-1.5">Timeline</span>
                <span className="text-[14px] font-semibold text-ink">Sep 2024 - Mar 2025</span>
              ...
              <div>
                ...
                <span className="text-[14px] font-semibold text-ink">Corporate Operations</span>
                <p className="text-[10px] text-muted-soft mt-1 leading-normal">
                  1 Designer, 1 Front-end Developer, 1 Content Writer.
                </p>
              </div>
              <div>
                <span className="block font-bold uppercase tracking-wider text-muted mb-1.5">Live Website</span>
                <a
                  href="https://jkvelvet.com/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-[14px] font-bold text-plum hover:underline"
                >
                  <span>jkvelvet.com</span>
                  <span className="text-[10px]">↗</span>
                </a>
                <p className="text-[10px] text-muted-soft mt-1 leading-normal">
                  View the active corporate platform online.
                </p>
              </div>
              <div>
                <span className="block font-bold uppercase tracking-wider text-muted mb-1.5">Tools &amp; Tech</span>
                <div className="flex flex-wrap gap-1 mt-1">
                  {["Figma", "FigJam", "Illustrator"].map((t) => (
                    <span
                      key={t}
                      className="inline-flex rounded-full border border-plum/20 bg-plum/5 px-2 py-0.5 font-semibold text-[10px] text-plum"
                    >
                      {t}
                    </span>
                  ))}
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left text-xs px-3 py-2 rounded transition-all duration-150 block truncate ${
                      isActive
                        ? "bg-plum/10 text-plum font-bold border-l-2 border-plum"
            ...
            {caseStudySections.map((sec, idx) => (
              ...
              >
                <Reveal delay={idx === 0 ? 50 : 0}>
                  ...
                  {/* Inline interactive sandbox mockup injected inside 'Inquiry Experience' section */}
                  {sec.id === "inquiry-experience" && (
                    <div className="mt-8 border border-hairline rounded-xl bg-canvas p-2 sm:p-4 shadow-lift">
                      <div className="mb-3 flex items-center justify-between border-b border-hairline pb-2 px-1">
                        ...
                        <span className="text-[10px] font-mono text-muted uppercase font-semibold">
                          Interactive JK Velvet Corporate RFQ Workspace
                        </span>
                      </div>
                      <CaseStudyPreview slug="industrial-websites" />
                    </div>
                  )}
                </Reveal>
              </section>
